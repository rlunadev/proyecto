...
<div class="panel panel-default">
	<!-- <div class="col-md-12">
      ...
      <div class="col-md-6">
        <div class="row">
           <div class="box box-danger">
              <div class="box-header with-border">
                <h3 class="box-title">Datos Generales Formula</h3>
              ...
              <div class="box-body">
                  ...
                  <div class="box-body">
                    <div class="form-group">
                      <label for="nombreFormula">Nombre Formula</label>
        ...
        <input type="hidden" id="precio_venta" class="form-control" value="">
...
var auxId='';
 $(document).ready(function () {
	$('.select2').select2();
  $(".tableSalida").load("http://localhost:8000/proyecto/calculo/public/partial/detalleformula.html");
  $('#datepicker').datepicker({autoclose: true})
  $("#cantidad").keyup(function(){
    var price_sale=parseFloat($("#precio_venta").val());
    var equivale=parseFloat($("#equivale").val())
    //console.log($(this).val()*(price_sale/equivale));
    $("#parcial").text(($(this).val()*(price_sale/equivale)).toFixed(2));
  });
 });
      ...
      success: function(result) {
        $.each(result.data, function() {
          $.each(this, function(index, value) {
            var newItem = $("<tr  id='trId_"+value.id+"' role='row' class='odd'><td class='sorting_1'>" + value.nombre + "</td><td>" + value.equivale+" </td><td>" + value.precio_venta+" </td><td class='text-right'><button type='button' class='btn btn-default btn-sm'  onclick='editFromTable("+value.id+")' data-href='"+value.id+"'  data-toggle='modal' data-target='#modal-edit' > Agregar </button></td></tr>");
...
//Get all DETALLE SALIDA WHERE STATUS=1
  function getAllDetalle(){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/formulaDetalle/GetAll?token="+localStorage.getItem('token'),
      //data:{data:2},
      success: function(result) {
        $.each(result.data, function() {
          $.each(this, function(index, value) {
            var newItem = $("<tr  id='trId_"+value.id+"' role='row' class='odd'><td class='sorting_1'> <input type='hidden'>" + value.parametro.nombre + "</td><td>" + value.categoria.nombre + "</td><td>" + value.parametro.unidad + "</td><td>" + value.cantidad+" </td><td >" + value.precio+" </td><td class='precio'>" + value.subTotal+" </td><td class='text-right'><button type='button' class='btn btn-default btn-sm' onclick='deleteFromTable("+value.id+")' data-href='"+value.id+"' data-toggle='modal' data-target='#confirm-modal'> Borrar </button></td></tr>");
              $("#tableDetalleFormula tbody").append(newItem);
          });
        });
        var newItem = $("<tfoot><tr><td></td><td></td><td></td><td></td><td><b>TOTAL</b></td><td><p id='total'></p> </td><td></td></tr></tfoot>");
...
//Delete By Id
  function deleteById (id){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/formulaDetalle/DeleteById?token="+localStorage.getItem('token'),
      data:{
        id:id
      },
      success: function(result) {
...
//SAVE DETALLE SALIDA DETALLE
  function SaveData (nombre,precio_venta,cantidad,categoria,unidad,parcial){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/formulaDetalle/SaveData?token="+localStorage.getItem('token'),
      data:{
        parametro_id:auxId,
        nombre:nombre,
        precio:precio_venta,
...
var idFormula='';
 function Saveformula (){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/formula/SaveData?token="+localStorage.getItem('token'),
      data:{
...
$(document).on('click', '#crearFormula', function (e) {
  ...
  $(document).on('click', '#okeyEdit', function (e) {
    SaveData($("#nombre").val(),$("#precio_venta").val(),$("#cantidad").val(),$("#select_categoria").val(),$("#unidad").val(),$("#parcial").text());
  });
function clear(){
