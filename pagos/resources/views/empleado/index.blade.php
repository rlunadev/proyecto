...
//Get all
  function getAll(){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/empleado/GetAll?token="+localStorage.getItem('token'),
      data:{data:2},
      success: function(result) {
        $.each(result.data, function() {
          $.each(this, function(index, value) {
              var newItem = $("<tr  id='trId_"+value.id+"' role='row' class='odd'><td class='sorting_1'> <input type='hidden'>" + value.nombre + "</td><td>" + value.cantidad+" </td><td>" + value.precio_unitario+" </td><td>" + value.precio_venta+" </td><td>" + value.unidad.nombre+" </td><td>" + value.categoria.nombre+" </td><td class='text-right'><button type='button' class='btn btn-default btn-sm'  onclick='editFromTable("+value.id+")' data-href='"+value.id+"'  data-toggle='modal' data-target='#modal-edit' > Edit </button> <button type='button' class='btn btn-default btn-sm'  onclick='deleteFromTable("+value.id+")' data-href='"+value.id+"' data-toggle='modal' data-target='#confirm-modal'> Delete </button></td></tr>");
...
//Delete By Id
  function deleteById (id){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/empleado/DeleteById?token="+localStorage.getItem('token'),
      data:{
        id:id
      },
      success: function(result) {
...
//Save Data
  function SaveData (nombre,cantidad,precio_unitario,precio_venta,unidad_id,categoria_id){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/empleado/SaveData?token="+localStorage.getItem('token'),
      data:{
        nombre:nombre,
        cantidad:cantidad,
      ...
      success: function(result) {
        var newItem = $("<tr  id='trId_"+result.id+"' role='row' class='odd'><td class='sorting_1'> <input type='hidden'>" +nombre + "</td><td>" + cantidad+" </td><td>" + precio_unitario+" </td><td>" + precio_venta+" </td><td>" + $("#select_unidad :selected").text()+" </td><td>" + $("#select_categoria :selected").text()+" </td><td class='text-right'><button type='button' class='btn btn-default btn-sm' onclick='editFromTable("+result.id+")' data-href='"+result.id+"'  data-toggle='modal' data-target='#modal-edit' > Edit </button> <button type='button' class='btn btn-default btn-sm' onclick='deleteFromTable("+result.id+")' data-href='"+result.id+"' data-toggle='modal' data-target='#confirm-modal'> Delete </button></td></tr>");
            $("#table1 tbody").append(newItem);
            clear();
            clearSelect();
      },
...
//Get By Id 
  function GetById (id){
    $.ajax({
      type: 'POST',
      url:{!!json_encode(url('/'))!!}+"/api/empleado/GetById?token="+localStorage.getItem('token'),
      data:{
        id:id
      },
      success: function(result) {
...
//Update
 function update (nombre,cantidad,precio_unitario,precio_venta,unidad_id,categoria_id){
    $.ajax({
      type: 'POST',
      ...
      success: function(result) {
        ...
        $('#trId_' + auxId).find("td").eq(2).html(result.data[0].precio_unitario);
...
//add=0, isedit=1
getSelectOption('tipoPago',0,0);
getSelectOption('tipoEmpleado',0,0);
...
//
  ...
  $(document).on('click', '#okeyEdit', function (e) {
    update($("#nombreEdit").val(),$("#cantidadEdit").val(), $("#precio_unitarioEdit").val(),$("#precio_ventaEdit").val(),$("#select_unidadEdit").val(), $("#select_categoriaEdit").val());
  });
function clear(){
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
              <label>Unidad</label>
              <select class="form-control select2" id="select_tipoPago" style="width: 100%;">

              </select>
          </div>
          <div class="form-group">
              <label>Categoria</label>
              <select class="form-control select2" id="select_tipoEmpleado" style="width: 100%;">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
              <label>Precio Unitario</label>
              <input type="number" step="0.1" id="precio_unitarioEdit" class="form-control" value="">
          </div>
          <div class="form-group">
              <label>Precio Venta</label>
          ...
          <div class="form-group">
              ...
              <select class="form-control select2" id="select_tipoPagoEdit" style="width: 100%;">
          ...
          <div class="form-group">
              ...
              <select class="form-control select2" id="select_tipoEmpleadoEdit" style="width: 100%;">
              </select>
          </div>
        </div>
        <div class="modal-footer">
