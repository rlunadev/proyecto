...
<div class="panel panel-default">
	<div class="col-md-12" style="background-color:#f5f5f5">
		<div class="col-md-8">
			<div class="panel-heading">TIPO EMPLEADO</div>
		</div>
		<div class="col-md-4">
...
function getAll(){
	$.ajax({
		type: 'POST',
		url:{!!json_encode(url('/'))!!}+"/api/tipoEmpleado/GetAll?token="+localStorage.getItem('token'),
		success: function(result) {
			$.each(result.data, function() {
				$.each(this, function(index, value) {
            		var newItem = $("<tr  id='trId_"+value.id+"' role='row' class='odd'><td class='sorting_1'> <input type='hidden'>" + value.nombre + "</td><td>" + value.descripcion+" </td><td class='text-right'><button type='button' class='btn btn-default btn-sm'  onclick='editFromTable("+value.id+")' data-href='"+value.id+"'  data-toggle='modal' data-target='#modal-edit' > Edit </button> <button type='button' class='btn btn-default btn-sm'  onclick='deleteFromTable("+value.id+")' data-href='"+value.id+"' data-toggle='modal' data-target='#confirm-modal'> Delete </button></td></tr>");
...
function deleteById (id){
	$.ajax({
		type: 'POST',
		url:{!!json_encode(url('/'))!!}+"/api/tipoEmpleado/DeleteById?token="+localStorage.getItem('token'),
    data:{
      id:id
    },
		success: function(result) {
...
function SaveData (nombre,descripcion){
	$.ajax({
		type: 'POST',
		url:{!!json_encode(url('/'))!!}+"/api/tipoEmpleado/SaveData?token="+localStorage.getItem('token'),
    data:{
      nombre:nombre,
...
function GetById (id){
	$.ajax({
		type: 'POST',
		url:{!!json_encode(url('/'))!!}+"/api/tipoEmpleado/GetById?token="+localStorage.getItem('token'),
    data:{
      id:id
    },
		success: function(result) {
...
function update (nombre,descripcion){
	$.ajax({
		type: 'POST',
		url:{!!json_encode(url('/'))!!}+"/api/tipoEmpleado/Update?token="+localStorage.getItem('token'),
    data:{
      id:auxId,
      nombre:nombre,
      descripcion:descripcion
